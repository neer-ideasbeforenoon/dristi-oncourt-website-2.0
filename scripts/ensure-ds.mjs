 ...
 *
 * Runs on `npm install` (postinstall). A new contributor needs only:
 *   git clone <this repo> && cd <it> && npm install && npm run dev
 *
 * Always installs to vendor/pucar-design-system (gitignored) so every machine has the
 * same in-repo path. Verifies org/remote; refuses wrong-org trees.
 *
 * Ported from dristi-app's root script. The one change: it imports resolve-ds with a
 * plain static import. The original used a dynamic `await import(join(..., "../apps/
 * dristi-app/scripts/resolve-ds.mjs"))` purely to cross the workspace boundary; here
 * both files sit in scripts/, so that hop would only be a way to get it wrong.
 */
...
function vendorReady() {
  return looksLikeDsTree(VENDOR_DS_PATH) && isAuthoritativeDs(VENDOR_DS_PATH);
}

/**
 * Put the vendor tree on the pinned commit.
 *
 * Best-effort by design — a failed sync must never fail an install, so every step may
 * fall through to "use what we have", loudly. Being offline is survivable; being
 * silently on the wrong design system is not, which is why check:ds-fresh exists as a
...
if (process.env.PUCAR_DS_ROOT) {
  ...
  console.error(`PUCAR_DS_ROOT is set to "${root}" but it is not an authoritative DS.`);
  console.error(
...
try {
  ...
  execFileSync("git", ["clone", "--filter=blob:none", CLONE_URL, VENDOR_DS_PATH], {
    stdio: "inherit",
  });
} catch {
  console.error(
