...
- **Audiences.** Nobody has done the research. The old site tells us which services
  exist, not who uses them or how.
- **Information architecture.** The 35 routes in `current-site-audit.md` are an
  inventory of what exists, not a proposal for what should. A real IA needs the audience
  work first.
- **Content model and editorial ownership.** Unknown: who writes a notice, who approves
  it, how often cause lists change, where the Malayalam copy comes from.
